...
class authService {
  ...
  searchParents(data){
    return http.get(`/users/search-parents?search=${data}`, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('token')
        }
      }
  ...
  searchParentsByClass(searchTerm, classId){
    return http.get(`/users/search-parents/by-class?search=${searchTerm}&classId=${classId}`, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('token')
        }
      }
