          ...
          <v-expansion-panel v-for="transaction in currentPaginatedTransactions" :key="transaction.id">
            ...
            <v-expansion-panel-text>
              <div>
                ...
                <v-tooltip text="Podajemy imię i nazwisko odbiorcy tylko jeśli jest on również klientem naszego portalu."
                  location="bottom">
    ...
    if (newValue) {
      const formattedTransactions = newValue.map((transaction) => {
        const isNegativeAmount =
          ...
          transaction.type === "FUNDRAISE_DEPOSIT" ||
          transaction.type === "FUNDRAISE_WITHDRAW_CORRECTION";

        return {
          ...transaction,
          type: transactionTypes[transaction.type] || transaction.type,
          amount: `${isNegativeAmount ? '-' : ''}${transaction.amount} zł`,
