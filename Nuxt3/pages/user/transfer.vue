<template>
  <v-sheet class="d-flex flex-wrap h-100">
    <v-card class="flex-1-0 ma-16 pa-2" :elevation="24" rounded>
      <v-card-title>
        <div class="text-h3 text-center font-weight-bold mb-16">
          Wykonaj przelew
        </div>
      </v-card-title>
      <v-card-text>
        ...
        <v-form @submit.prevent="handleTransfer" ref="transfer" v-model="valid">
          <v-text-field
            ...
            variant="outlined"
            label="Na rachunek"
            v-model="transferDetails.accountNumberDest"
          ...
          <div class="text-center">
            <v-btn class="mr-4" color="red" @click="$refs.transfer.reset()"
              >Wyczyść</v-btn
            >
...
const accountInfo = computed(() => {
  return user
    ? `${formatAccountNumber(user.accountNumber)} (${user.balance} PLN)`
    : "";
});
const accountNumberRules = [
  (v) => !!v || "Pole nie może być puste",
  (v) =>
    v.replace(/\s/g, "").length === 26 || "Numer rachunku musi mieć 26 znaków",
  (v) =>
    /^\d+$/.test(v.replace(/\s/g, "")) ||
    "Numer rachunku musi składać się z samych cyfr",
  (v) =>
    v.replace(/\s/g, "") !== user.accountNumber ||
    "Nie możesz wykonać przelewu na własne konto",
];
const amountRules = [
  (v) => !!v || "Pole nie może być puste",
...
const handleTransfer = async () => {
  transferDetails.accountNumberDest = transferDetails.accountNumberDest.replace(
    /\s/g,
    ""
  );
  await transactionStore
    .sendTransfer(transferDetails)
    .then(() => {
