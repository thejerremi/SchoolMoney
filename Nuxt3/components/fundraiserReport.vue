<template>
  <div class="d-flex flex-column justify-center align-center text-center">
    <h2>Wygeneruj raporty finansowe z tej zbiórki dla wszystkich rodziców</h2>
    <v-btn class="mt-3" variant="outlined" @click="generateAll">Wygeneruj</v-btn>
    <div class="mt-6">
      <h2>Wygeneruj raporty finansowe z tej zbiórki dla konkretnego rodzica</h2>
...
const generateAll = async () => {
  try {
    ...
    const response = await reportService.fetchAllTransactionsFromFundraiser(route.params.id);
...
const generateForParent = async () => {
  try {
    snackbarInfo('Trwa generowanie raportu...', 150000)
    const response = await reportService.fetchParentTransactionsFromFundraiser(route.params.id, selectedParent.value);
