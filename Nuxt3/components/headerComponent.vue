<template>
  <v-app-bar ref="appBar">
    ...
    <v-app-bar-title class="cursor-pointer font-weight-bold" @click="navigateTo('/')">SchoolMoney</v-app-bar-title>

    <v-btn v-if="authStore.user && authStore.user.role===`ADMIN`" variant="text" @click="navigateTo('/user/chat')">Czat</v-btn>
    <v-btn v-if="authStore.user && authStore.user.role===`ADMIN`" variant="text" @click="navigateTo('/admin/admin-panel')">Panel admina</v-btn>
