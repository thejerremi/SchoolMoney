<template>
  <div class="d-flex flex-column">
    <h1 class="text-center font-weight-bold" v-if="!isTreasurer">Ustawienia dostępne tylko dla skarbnika klasy</h1>
    <h2 class="text-center font-weight-bold font-italic" v-if="authStore.user.role === 'ADMIN'">Masz dostęp do ustawień, ponieważ jesteś administratorem</h2>
    <h3 class="font-weight-bold">Dodaj rodzica</h3>
    <div class="d-flex ga-5">
      <v-autocomplete :disabled="!isTreasurer && authStore.user.role !== 'ADMIN'" v-model="selectedParent" :items="parents" :loading="loading"
        v-model:search="search" :hide-no-data="true" label="Dodaj rodzica" :item-title="getParentFullName"
        item-value="id" hide-details></v-autocomplete>
      <v-btn v-if="selectedParent" class="align-self-center" color="green" @click="addParent()">Dodaj</v-btn>
    </div>
    <div class="mt-12">
      <h3 class="font-weight-bold">Zmień nazwę klasy</h3>
    <div class="d-flex ga-4">
      <v-text-field :disabled="!isTreasurer && authStore.user.role !== 'ADMIN'" class="" label="Nazwa klasy:" v-model="classDataCopy.className" />
      <v-btn v-if="classData.className !== classDataCopy.className" class="align-self-center mb-6" color="green"
        @click="updateClassName()">Zapisz</v-btn>
    </div>
    </div>
    <div class="mt-12">
      <h3 class="font-weight-bold">Zmień skarbnika klasy</h3>
    <div class="d-flex ga-5">
      <v-autocomplete :disabled="!isTreasurer && authStore.user.role !== 'ADMIN'" v-model="selectedTreasurer" :items="classDataCopy.parents" :loading="loading"
        v-model:search="searchTreasurer" :hide-no-data="true" label="Zmień skarbnika" :item-title="getParentFullName"
        item-value="id" hide-details></v-autocomplete>
      <v-btn v-if="selectedTreasurer && (selectedTreasurer !== classDataCopy.treasurer.id)" class="align-self-center" color="green" @click="updateClassTreasurer()">Zmień</v-btn>
    </div>
    </div>
    <div class="py-12">
      <h3 class="font-weight-bold">Usuń klasę</h3>
      <div class="d-flex flex-column justify-center">
    <v-btn :disabled="!isTreasurer && authStore.user.role !== 'ADMIN'" color="red" @click="showConfirmDeleteButtons = true">
      Zmień
    </v-btn>
    ...
    <v-slide-y-transition>
      <div v-if="showConfirmDeleteButtons" class="d-flex justify-center mt-3">
        <v-btn color="green" @click="confirmDelete" @mouseenter="showWarning = true" @mouseleave="showWarning = false">
          Potwierdź
        </v-btn>
        <v-btn color="red" class="ml-2" @click="showConfirmDeleteButtons = false;">
          Anuluj
        </v-btn>
      </div>
    </v-slide-y-transition>
